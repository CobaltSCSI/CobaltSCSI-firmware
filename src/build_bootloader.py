...
# It is basically a copy of the main firmware but using CobaltSCSI_bootloader.cpp
...
# Build a version of CobaltSCSI_main.cpp that calls bootloader instead
env2 = env.Clone()
env2.Append(CPPFLAGS = "-DCOBALTSCSI_BOOTLOADER_MAIN")
bootloader_main = env2.Object(
    os.path.join("$BUILD_DIR", "bootloader_main.o"),
    ["CobaltSCSI_main.cpp"]
)

# Include all other dependencies except CobaltSCSI_main.cpp
dep_objs = []
for nodelist in env["PIOBUILDFILES"]:
    for node in nodelist:
        filename = str(node.rfile())
        if 'CobaltSCSI_main' not in filename:
