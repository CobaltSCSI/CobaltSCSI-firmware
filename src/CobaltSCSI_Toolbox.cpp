...
**/
#include "CobaltSCSI_Toolbox.h"
#include "CobaltSCSI_disk.h"
#include "CobaltSCSI_cdrom.h"
...
extern "C" int8_t scsiToolboxEnabled()
{
    ...
    if (enabled == -1)
    {
        ...
        dbgmsg("CobaltSCSI Toolbox enabled = ", enabled);
    }
    return enabled == 1;
}
...
extern "C" int scsiToolboxCommand()
{
    ...
    if (unlikely(command == COBALTSCSI_TOOLBOX_COUNT_FILES))
    {
        char img_dir[MAX_FILE_PATH];
        dbgmsg("COBALTSCSI_TOOLBOX_COUNT_FILES");
        getToolBoxSharedDir(img_dir);
        doCountFiles(img_dir);
    }
    else if (unlikely(command == COBALTSCSI_TOOLBOX_LIST_FILES))
    {
        char img_dir[MAX_FILE_PATH];
        dbgmsg("COBALTSCSI_TOOLBOX_LIST_FILES");
        getToolBoxSharedDir(img_dir);
        onListFiles(img_dir);
    }
    else if (unlikely(command == COBALTSCSI_TOOLBOX_GET_FILE))
    {
        char img_dir[MAX_FILE_PATH];
        dbgmsg("COBALTSCSI_TOOLBOX_GET_FILE");
        getToolBoxSharedDir(img_dir);
        onGetFile10(img_dir);
    }
    else if (unlikely(command == COBALTSCSI_TOOLBOX_SEND_FILE_PREP))
    {
        char img_dir[MAX_FILE_PATH];
        dbgmsg("COBALTSCSI_TOOLBOX_SEND_FILE_PREP");
        getToolBoxSharedDir(img_dir);
        onSendFilePrep(img_dir);
    }
    else if (unlikely(command == COBALTSCSI_TOOLBOX_SEND_FILE_10))
    {
        dbgmsg("COBALTSCSI_TOOLBOX_SEND_FILE_10");
        onSendFile10();
    }
    else if (unlikely(command == COBALTSCSI_TOOLBOX_SEND_FILE_END))
    {
        dbgmsg("COBALTSCSI_TOOLBOX_SEND_FILE_END");
        onSendFileEnd();
    }
    else if(unlikely(command == COBALTSCSI_TOOLBOX_TOGGLE_DEBUG))
    {
        dbgmsg("COBALTSCSI_TOOLBOX_TOGGLE_DEBUG");
        onToggleDebug();
    }
    else if(unlikely(command == COBALTSCSI_TOOLBOX_LIST_CDS))
    {
        char img_dir[4];
        dbgmsg("COBALTSCSI_TOOLBOX_LIST_CDS");
        snprintf(img_dir, sizeof(img_dir), CD_IMG_DIR, static_cast<int>(img.scsiId) & S2S_CFG_TARGET_ID_BITS);
        onListFiles(img_dir, true);
    }
    else if(unlikely(command == COBALTSCSI_TOOLBOX_SET_NEXT_CD))
    {
        char img_dir[4];
        dbgmsg("COBALTSCSI_TOOLBOX_SET_NEXT_CD");
        snprintf(img_dir, sizeof(img_dir), CD_IMG_DIR, static_cast<int>(img.scsiId) & S2S_CFG_TARGET_ID_BITS);
        onSetNextCD(img_dir);
    }
    else if(unlikely(command == COBALTSCSI_TOOLBOX_LIST_DEVICES))
    {
        dbgmsg("COBALTSCSI_TOOLBOX_LIST_DEVICES");
        onListDevices();
    }
    else if (unlikely(command == COBALTSCSI_TOOLBOX_COUNT_CDS))
    {
        char img_dir[4];
        dbgmsg("COBALTSCSI_TOOLBOX_COUNT_CDS");
        snprintf(img_dir, sizeof(img_dir), CD_IMG_DIR, static_cast<int>(img.scsiId) & S2S_CFG_TARGET_ID_BITS);
