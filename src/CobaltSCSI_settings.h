...
// must be in the same order as speed_grade_strings[]  in CobaltSCSI_settings.cpp
typedef enum
{
...
typedef struct __attribute__((__packed__)) scsi_system_settings_t
{
    ...
    uint8_t speedGrade; // memory allocation for cobaltscsi_speed_grade_t enum
...
public:
    ...
    cobaltscsi_speed_grade_t stringToSpeedGrade(const char *speed_grade_str, size_t length);
...
extern CobaltSCSISettings g_scsi_settings;
#endif // __cplusplus
