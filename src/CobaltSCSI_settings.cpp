/**
 * This file is originally part of ZuluSCSI adopted for CobaltSCSI
 *
...
CobaltSCSISettings g_scsi_settings;
...
// must be in the same order as cobaltscsi_speed_grade_t in CobaltSCSI_settings.h
...
const char **CobaltSCSISettings::deviceInitST32430N(uint8_t scsiId)
{
...
void CobaltSCSISettings::setDefaultDriveInfo(uint8_t scsiId, const char *presetName, S2S_CFG_TYPE type)
{
...
scsi_system_settings_t *CobaltSCSISettings::initSystem(const char *presetName)
{
    scsi_system_settings_t &cfgSys = m_sys;
    ...
    cfgSys.speedGrade = cobaltscsi_speed_grade_t::SPEED_GRADE_DEFAULT;
...
scsi_device_settings_t* CobaltSCSISettings::initDevice(uint8_t scsiId, S2S_CFG_TYPE type)
{
...
scsi_system_settings_t *CobaltSCSISettings::getSystem()
{
    return &m_sys;
}

scsi_device_settings_t *CobaltSCSISettings::getDevice(uint8_t scsiId)
{
    return &m_dev[scsiId];
}

scsi_system_preset_t CobaltSCSISettings::getSystemPreset()
{
    return m_sysPreset;
}

const char* CobaltSCSISettings::getSystemPresetName()
{
    return systemPresetName[m_sysPreset];
}

scsi_device_preset_t CobaltSCSISettings::getDevicePreset(uint8_t scsiId)
{
    return m_devPreset[scsiId];
}

const char* CobaltSCSISettings::getDevicePresetName(uint8_t scsiId)
{
    return devicePresetName[m_devPreset[scsiId]];
}


cobaltscsi_speed_grade_t CobaltSCSISettings::stringToSpeedGrade(const char *speed_grade_target, size_t length)
{
    cobaltscsi_speed_grade_t grade = cobaltscsi_speed_grade_t::SPEED_GRADE_DEFAULT;
    ...
    for (uint8_t i = 0; i < sizeof(speed_grade_strings)/sizeof(speed_grade_strings[0]); i++)
    {
        ...
        {
            grade = (cobaltscsi_speed_grade_t)i;
...
const char *CobaltSCSISettings::getSpeedGradeString()
{
    return speed_grade_strings[m_sys.speedGrade];
}


const bool CobaltSCSISettings::isEjectButtonSet()
{
    bool is_set = false;
