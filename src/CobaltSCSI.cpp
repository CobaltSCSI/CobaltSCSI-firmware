...
#include "CobaltSCSI_log_trace.h"
#include "CobaltSCSI_settings.h"
#include "CobaltSCSI_disk.h"
#include "CobaltSCSI_initiator.h"
#include "CobaltSCSI_msc_initiator.h"
#include "CobaltSCSI_msc.h"
#include "CobaltSCSI_blink.h"
#include "ROMDrive.h"
...
{
#ifdef COBALTSCSI_HARDWARE_CONFIG
...
bool findHDDImages()
{
...
#endif // COBALTSCSI_HARDWARE_CONFIG
  ...
  if (removable_count == 1)
  {
    ...
    {
      logmsg("Found 1 removable device, to set an eject button see EjectButton in the '", CONFIGFILE,"', or the https://github.com/CobaltSCSI/CobaltSCSI-firmware/wiki/Buttons");
    }
  }
  else if (removable_count > 1)
  {
    ...
    {
      logmsg("Multiple removable devices, to set an eject button see EjectButton in the '", CONFIGFILE,"', or the https://github.com/CobaltSCSI/CobaltSCSI-firmware/wiki/Buttons");
    }
  }
#endif // COBALTSCSI_BUTTONS
  return foundImage;
...
{
#if defined(COBALTSCSI_HARDWARE_CONFIG)
  ...
  if (platform_network_supported()) {
    if (scsiDiskCheckAnyNetworkDevicesConfigured())
    {
      ...
      if (scsiDev.boardCfg.wifiSSID[0] != '\0')
        ...
        logmsg("No Wi-Fi SSID or Password found. Use the Wi-Fi DA to configure the network.");
    }
    else
    {
...
static void check_for_unused_update_files()
{
  ...
  while (file.openNext(&root, O_RDONLY))
  {
    if (!file.isDir())
    {
      ...
      if (strncasecmp(filename, "cobaltscsi", sizeof("cobaltscsi" - 1)) == 0 &&
  ...
  if (bin_files_found)
  {
    ...
    logmsg("See https://github.com/CobaltSCSI/CobaltSCSI-firmware/wiki/Updating-Firmware for more information");
  }
}
...
static void cobaltscsi_setup_sd_card(bool wait_for_card = true)
{
  ...
  if (g_sdcard_present)
  {
    ...
    cobaltscsi_speed_grade_t speed_grade = (cobaltscsi_speed_grade_t) g_scsi_settings.getSystem()->speedGrade;
    if (speed_grade != cobaltscsi_speed_grade_t::SPEED_GRADE_DEFAULT)
    { 
      logmsg("Speed grade set to ", g_scsi_settings.getSpeedGradeString(), " reclocking system");
      if (platform_reclock(speed_grade))
      {
        logmsg("======== Reinitializing CobaltSCSI after reclock ========");
        g_sdcard_present = mountSDCard();
      }
    }
    else
...
extern "C" void cobaltscsi_setup(void)
{
  platform_init();
  platform_late_init();
  ...
  cobaltscsi_setup_sd_card(!is_initiator);
  ...
  if (check_mass_storage && !is_initiator)
  {
    ...
    )
    {
      ...
      {
        cobaltscsi_msc_loop();
        logmsg("Re-processing filenames and cobaltscsi.ini config parameters");
        cobaltscsi_setup_sd_card();
      }
    }
  }
#endif
...
extern "C" void cobaltscsi_main_loop(void)
{
