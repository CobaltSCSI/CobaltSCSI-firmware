...
/*******************************************************/
static void cobalt_pio_remove_program(PIO pio, const pio_program_t *program, uint loaded_offset, bool &removed)
{
    if (!removed)
    {
...
{
    cobalt_pio_remove_program(SCSI_DMA_PIO,
    ...
    uint8_t delay = g_cobaltescsi_timings->scsi.req_delay - 2;
    assert( delay <= 0xF);
...
void scsi_accel_rp2040_init()
{
    ...
    if (g_channels_claimed) {
        ...
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_parity_program, g_scsi_dma.pio_offset_parity, g_scsi_dma.pio_removed_parity);
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_accel_async_write_program, g_scsi_dma.pio_offset_async_write, g_scsi_dma.pio_removed_async_write);
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_sync_write_pacer_program, g_scsi_dma.pio_offset_sync_write_pacer, g_scsi_dma.pio_removed_sync_write_pacer);
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_accel_read_program, g_scsi_dma.pio_offset_read, g_scsi_dma.pio_removed_read);
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_sync_read_pacer_program, g_scsi_dma.pio_offset_sync_read_pacer, g_scsi_dma.pio_removed_sync_read_pacer);
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_read_parity_program, g_scsi_dma.pio_offset_read_parity, g_scsi_dma.pio_removed_read_parity);
        cobalt_pio_remove_program(SCSI_DMA_PIO, &scsi_sync_write_program, g_scsi_dma.pio_offset_sync_write, g_scsi_dma.pio_removed_sync_write);
...
bool scsi_accel_rp2040_setSyncMode(int syncOffset, int syncPeriod)
{
    ...
    if (syncOffset != g_scsi_dma.syncOffset || syncPeriod != g_scsi_dma.syncPeriod)
    {
        ...
        if (syncOffset > 0)
        {
            ...
            uint32_t up_rounder = g_cobaltscsi_timings->scsi.clk_period_ps / 2 + 1;
            uint32_t delay_in_ps = (syncPeriod * 4) * 1000;
            // This is the period in clock cycles rounded up
            int totalPeriod = (delay_in_ps + up_rounder) / g_cobaltscsi_timings->scsi.clk_period_ps;
            ...
            if (syncPeriod < 25)
            {
                ...
                totalPeriod += g_cobaltscsi_timings->scsi_20.total_period_adjust;
                delay0 = g_cobaltscsi_timings->scsi_20.delay0; //Data setup time, should be min 11.5ns according to the spec for FAST-20
                delay1 = g_cobaltscsi_timings->scsi_20.delay1; //pulse width, should be min 15ns according to the spec for FAST-20
                ...
                rdelay1 = g_cobaltscsi_timings->scsi_20.rdelay1;
                rtotalPeriod += g_cobaltscsi_timings->scsi_20.rtotal_period_adjust;
            }
            else if (syncPeriod < 50 )
            {
                // Fast-10 SCSI timing: 30 ns assertion period, 25 ns skew delay
                // The hardware rise and fall time require some extra delay,
                totalPeriod += g_cobaltscsi_timings->scsi_10.total_period_adjust;
                delay0 = g_cobaltscsi_timings->scsi_10.delay0; // 4;
                delay1 = g_cobaltscsi_timings->scsi_10.delay1; // 6;
                delay2 = totalPeriod - delay0 - delay1 - 3;
                if (delay2 < 0) delay2 = 0;
                if (delay2 > 15) delay2 = 15;
                rdelay1 = g_cobaltscsi_timings->scsi_10.rdelay1;
                rtotalPeriod += g_cobaltscsi_timings->scsi_10.rtotal_period_adjust;
            }
            else
            {
                ...
                totalPeriod += g_cobaltscsi_timings->scsi_5.total_period_adjust;
                delay0 = g_cobaltscsi_timings->scsi_5.delay0;
                delay1 = g_cobaltscsi_timings->scsi_5.delay1;
                delay2 = totalPeriod - delay0 - delay1 - 3;
                if (delay2 < 2) delay2 = 2;
                if (delay2 > 15) delay2 = 15;
                rdelay1 = g_cobaltscsi_timings->scsi_5.rdelay1;
                rtotalPeriod += g_cobaltscsi_timings->scsi_5.rtotal_period_adjust;
            }
