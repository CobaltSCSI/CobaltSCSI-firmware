...
extern "C" bool scsiStatusSEL()
{
    if (g_scsi_ctrl_bsy)
    {
        ...
        // On RP2040 hardware the ATN signal is only availafe after OUT_BSY enables
...
extern "C" uint32_t scsiEnterPhaseImmediate(int phase)
{
    if (phase != g_scsi_phase)
    {
        ...
        if (scsiDev.compatMode < COMPAT_SCSI2 && (phase == DATA_IN || phase == DATA_OUT))
        {
            ...
            // after a command. The code in CobaltSCSI_disk.cpp tries to do this while waiting
