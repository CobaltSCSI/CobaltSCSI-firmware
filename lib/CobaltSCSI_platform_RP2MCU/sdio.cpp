...
void rp2040_sdio_init(int clock_divider)
{
    ...
    for (uint8_t i = 0; i < sizeof(sdio_cmd_clk_program_instructions) / sizeof(sdio_cmd_clk_program_instructions[0]); i++)
    {
        uint16_t instr = sdio_cmd_clk_program_instructions[i]
            | ((i & 1) ? pio_encode_delay(g_cobaltscsi_timings->sdio.delay0) : pio_encode_delay(g_cobaltscsi_timings->sdio.delay1));
    ...
    uint16_t instr = pio_encode_wait_gpio(true, SDIO_CLK - SDIO_BASE_OFFSET) | pio_encode_delay(g_cobaltscsi_timings->sdio.clk_div_pio - 1);
    temp_program_instr[2] = instr;
    // in PINS, 4                 [0]; [CLKDIV-2]
    instr = sdio_data_rx_program_instructions[3] | pio_encode_delay(g_cobaltscsi_timings->sdio.clk_div_pio - 2);
    ...
    instr = pio_encode_wait_gpio(true, SDIO_CLK - SDIO_BASE_OFFSET) | pio_encode_delay(g_cobaltscsi_timings->sdio.clk_div_pio + g_cobaltscsi_timings->sdio.delay1 - 1);
    ...
    {    
        uint16_t instr = sdio_data_tx_program_instructions[i]
            | ((i & 1) ? pio_encode_delay(g_cobaltscsi_timings->sdio.delay1) : pio_encode_delay(g_cobaltscsi_timings->sdio.delay0));
        temp_program_instr[i] = instr;
    }
