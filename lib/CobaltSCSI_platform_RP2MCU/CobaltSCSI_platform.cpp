...
#  ifdef COBALTSCSI_RM2
#    include <pico/cyw43_driver.h>
#  endif
#endif // COBALTSCSI_NETWORK

#ifdef PLATFORM_MASS_STORAGE
#include "CobaltSCSI_platform_msc.h"
#endif
...
static void reclock() {
    ...
    // reset PLL
    pll_init(pll_sys,
        g_cobaltscsi_timings->pll.refdiv,
        g_cobaltscsi_timings->pll.vco_freq,
        g_cobaltscsi_timings->pll.post_div1,
        g_cobaltscsi_timings->pll.post_div2);

    // switch clocks back to pll_sys
    clock_configure(clk_sys,
            CLOCKS_CLK_SYS_CTRL_SRC_VALUE_CLKSRC_CLK_SYS_AUX,
            CLOCKS_CLK_SYS_CTRL_AUXSRC_VALUE_CLKSRC_PLL_SYS,
            g_cobaltscsi_timings->clk_hz,
            g_cobaltscsi_timings->clk_hz);
    clock_configure(clk_peri,
            0,
...
bool platform_reclock(cobaltscsi_speed_grade_t speed_grade)
{
    CustomTimings ct;
    ...
    if (speed_grade != SPEED_GRADE_DEFAULT)
    {
        ...
        if (do_reclock)
        {
            ...
            if (g_cobaltscsi_timings->audio.audio_clocked)
            ...
            logmsg("Reclocking the MCU to ",(int) g_cobaltscsi_timings->clk_hz / MHZ, "MHz");
#ifndef SD_USE_RP2350_SDIO
            logmsg("Setting the SDIO clock to ", (int)((g_cobaltscsi_timings->clk_hz / g_cobaltscsi_timings->sdio.clk_div_pio + (5 * MHZ / 10)) / MHZ) , "MHz");
#endif
            usb_log_poll();
            reclock();
...
# if defined(COBALTSCSI_PICO) || defined(COBALTSCSI_PICO_2)
...
#if defined(COBALTSCSI_PICO) || defined(COBALTSCSI_PICO_2)
        logmsg("Debug logging can only be enabled via INI file \"DEBUG=1\" under [SCSI] in cobaltscsi.ini");
        logmsg("-- DEBUG DIP switch setting is ignored on CobaltSCSI Pico FS Rev. 2023b and 2023c boards");
        g_log_debug = false;
#endif
...
#if  defined(COBALTSCSI_NETWORK) && ! defined(COBALTSCSI_RM2)
